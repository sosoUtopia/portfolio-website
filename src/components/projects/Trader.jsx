import React from 'react'

const Trader = () => {
  return (
    <div className="project-data">
      <h1 className="project-title">
        Social Media Sentiment Paper Trader
      </h1>
      <h3 className="project-subtitle">Spring 2023 - Present</h3>
      <p className="project-description"> 
      Use Reddit and Twitter API to extract social media data regarding the stock market through sentiment
      analysis and interacts with the Alpaca Paper Trader with functional frontend and backend Utilized Django, Django
      </p>
      <p>
      Utilize Django, Django REST, Python, React, Typescript, HTML, CSS, TextBlob, Tweepy, Pandas, Numpy, and Praw
      </p>
      
    </div>  )
}

export default Trader